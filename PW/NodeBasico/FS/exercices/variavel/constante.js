...
const server = http.createServer(async (req, res) => {
  ...
  const parametroCirculo = urlInfo.query;

  if (!parametroCirculo.raio) {
    res.write(`
      <form action="" method="get" class="container">
      <div class="sub-container">
        <label class="text-titulo" for="raio">Entre com o raio do circulo:</label>
        <input class="input-text" type="text" name="raio" id="raio" />
        <br />
    ...
    const raio = Number(parametroCirculo.raio);
    const area = PI * (raio * raio);
    res.end(`<div class="container"><h1 class="apresentacao">A area do circulo é ${area}</h1></div>`);
  }
});
